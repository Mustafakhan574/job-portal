...
const Seeapplyjobs = () => {
  ...
  let {usersaccounts,setusersaccounts,server} = useContext(authdata)
  const userdelete=async(id)=>{
          try{
let result = await axios.get(`${server}/users/deleteuser/${id}`,{
...
setusersaccounts(result.data);
  ...
  return (
    <div className='w-full min-h-screen bg-[#540505]  flex flex-col items-center gap-4 text-white '>
     ...
     <h1 className='text-white text-[40px] text-center pt-[30px]'>Users</h1>
     <div className='w-[98%] flex flex-col sm:w-[70%] mb-[15px]'>
        {usersaccounts && usersaccounts.map((user)=>(
          ...
          <button className='bg-[#fb1b1b] px-4 rounded-2xl hover:text-white'onClick={()=>userdelete(user._id)}>Delete</button>
          </div>
        ))}
     </div>
    </div>
